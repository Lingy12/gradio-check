...
def save_annotation(label, id, text, saving_path):
    ...
    id = gr.Text(selected_index)
    text = gr.Text(ds[selected_index]['sentence'])
...
with gr.Blocks(theme='soft') as demo:
    ...
    with gr.Row():
        ...
        selected_index = np.random.choice(len(ds))
        id = gr.Textbox(selected_index, label='id', show_label=True, interactive=False)
        pg = gr.Text(f'{num_of_labels}/{len(ds)}', label='label progress', show_label=True)

    with gr.Row():
        text = gr.Text(ds[selected_index]['sentence'], label='transcription', show_label=True,interactive=True )
        audio = gr.Audio((ds[selected_index]['audio']['sampling_rate'], ds[selected_index]['audio']['array']))
